...
class Encoder(object):
    ...
    def encode(self, bboxes_in, labels_in, criteria=0.5):
        """
        encode:
            input  : bboxes_in (Tensor nboxes x 4, GT boxes), labels_in (Tensor nboxes)
            output : bboxes_out (Tensor 8732 x 4), labels_out (Tensor 8732)
            criteria : IoU threshold of bboexes

            Main algorithm codes verify as follows:
            >>> ious = torch.randn(4, 8)
            >>> ious
                tensor([[ 1.5297,  0.9348,  0.9498,  0.1387, -0.9470,  0.2210, -0.0385, -0.0909],
                        [-0.2596, -0.3972,  1.0702, -1.5128,  0.4353, -0.8624, -1.0820, -0.2580],
                        [-0.6447,  0.2284, -0.0180, -0.6265,  1.0532,  0.2533, -1.1198, -1.0419],
                        [-0.0269, -0.1796, -0.2935, -0.4850,  0.6669,  1.2316,  1.4718, -0.3114]])
            >>> best_dbox_ious, best_dbox_idx = ious.max(dim=0)
            >>> best_dbox_ious
                tensor([ 1.5297,  0.9348,  1.0702,  0.1387,  1.0532,  1.2316,  1.4718, -0.0909])
            >>> best_dbox_idx
                tensor([0, 0, 1, 0, 2, 3, 3, 0])
            >>> best_bbox_ious, best_bbox_idx = ious.max(dim=1)
            >>> best_bbox_ious
                tensor([1.5297, 1.0702, 1.0532, 1.4718])
            >>> best_bbox_idx
                tensor([0, 2, 4, 6])
            >>> best_dbox_ious.index_fill_(0, best_bbox_idx, 2.0)
                tensor([ 2.0000,  0.9348,  2.0000,  0.1387,  2.0000,  1.2316,  2.0000, -0.0909])
            >>> idx = torch.arange(0, best_bbox_idx.size(0), dtype=torch.int64)
            >>> idx
                tensor([0, 1, 2, 3])
            >>> best_bbox_idx.size(0)
                4
            >>> best_bbox_idx[idx]
                tensor([0, 2, 4, 6])
        ...
        # [8732,]    每个锚框最可能的类型
        best_dbox_ious, best_dbox_idx = ious.max(dim=0)  # 寻找每个default box匹配到的最大IoU(最佳GT)
        # [nboxes,]  每个类型对应的最佳锚框
        best_bbox_ious, best_bbox_idx = ious.max(dim=1)  # 寻找每个GT匹配到的最大IoU(最佳锚框)
